...
Only the action URL plus the email and password entry IDs are required; the others are optional. The app will post the data in `no-cors` mode so the form response is stored even when hosted on Vercel.
